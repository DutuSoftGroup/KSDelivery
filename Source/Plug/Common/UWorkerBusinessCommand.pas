...
type
  ...
  protected
    ...
    //系统是否已过期
    function SaveTruckIn(var nData: string): Boolean;
    //保存车辆进厂记录
  public
...
begin
  ...
  case FIn.FCommand of
   ...
   cBC_SaveTruckIn         : Result := SaveTruckIn(nData);
   else
    begin
      Result := False;
...
//Date: 2016/8/7
//Parm: 车牌号码(FIn.FData);磁卡编号(FIn.FExtParam)
//Desc: 执行抓拍
function TWorkerBusinessCommander.SaveTruckIn(var nData: string): Boolean;
var nStr: string;
...
begin
  ...
  try
    FListC.Clear;
    FListC.Values['Group'] :=sFlag_SerailSYS;
    FListC.Values['Object'] := sFlag_TruckLog;
    //to get serial no
    ...
    nStr := MakeSQLByStr([SF('T_ID', nOut.FData),
            ...
            ], sTable_TruckLog, '', True);
    gDBConnManager.WorkerExec(FDBConn, nStr);
    //插入门岗记录

    nStr := MakeSQLByStr([SF('T_LastTime', sField_SQLServer_Now, sfVal)],
            sTable_Truck, SF('T_Truck', FIn.FData), False);
    gDBConnManager.WorkerExec(FDBConn, nStr);
    //更新车辆时间
